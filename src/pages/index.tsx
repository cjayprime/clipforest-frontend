...
import { useAppConfig, useVideos } from '@/lib/hooks';
...
export default function Dashboard() {
  ...
  return (
    <PageContainer>
      <Head>
        <title>Home · ClipForest</title>
      </Head>
      <div className="flex flex-col gap-10">
        <section className="space-y-2">
          <span className="inline-flex items-center gap-2 rounded-full border border-outline-variant/30 bg-surface-high px-2.5 py-1 font-mono text-[10px]">
            <span className="size-2 animate-pulse rounded-full bg-tertiary" aria-hidden />
            <span className="text-tertiary">Pipeline {cfg?.pipelineVersion ?? '…'}</span>
          </span>
          <h1 className="text-3xl font-semibold tracking-tight text-fg sm:text-4xl">Create clips people want to watch</h1>
        ...
        <section className="space-y-5" aria-labelledby="recent-heading">
          <div className="flex flex-col justify-between gap-3 border-b border-outline-variant/20 pb-3 md:flex-row md:items-center">
            <div className="flex items-center gap-3">
              ...
              <span className="rounded-full bg-surface-high px-2 py-0.5 font-mono text-[10px] text-outline">{videos.data?.length ?? 0} total</span>
            </div>
            <div className="flex items-center gap-3">
              <Segmented
                label="Filter videos"
