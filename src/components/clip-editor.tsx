...
import type { CaptionPreset, FramingMode, Render } from '@/lib/types';
import { PageContainer } from './shell';
import { SourcePlayer, type SourcePlayerHandle } from './source-player';
import { Badge, Button, EmptyState, ErrorPanel, inputClass, ProgressBar, RenderStatusBadge, ScoreBadge, Segmented, Spinner, Toggle } from './ui';
...
export function ClipEditor({ id }: { id: string }) {
...
function EditorBody({ r, refetch }: { r: Render; refetch: () => void }) {
  ...
  const rerender = () =>
    run('rerender', async () => {
      const next = await api<Render>(`/renders/${r.id}/rerender`, {
        method: 'POST',
        json: { startMs, endMs, framingMode: framing, captions: { enabled: captionsOn, preset }, title: title.trim() || undefined },
      });
  ...
  return (
    <div className="flex flex-col">
      <Head>
        <title>{`${r.title} · ClipForest`}</title>
      </Head>
